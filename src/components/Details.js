...
export default class Details extends Component {
    ...
    componentDidMount() {
        ...
        axios
            ...
            .then(res => {
                const menuItems=res.data.menu.map(item=> item._id ? {...item,quantity:0} : null)
                this.setState({
                    menu: menuItems
                });
    ...
    addItemHandler = (item) => {
        const {totalPrice,menu} = this.state;
         const newMenu = menu.map(it=>it._id==item._id ? {...it,quantity:it.quantity+1} : 
            it )
            this.setState({
    ...
    paymentHandler = () => {
        ...
        const items= this.state.menu.filter(item=>item.quantity!=0)
            ...
            <div className="container details">
            {
                ...
                <>
                    ...
                    <h2>
                        ...
                        <div className="row ">
                        <h3 className="col-6">{ restaurant.name }</h3>
                        <h5 className="col-2"> Quantity </h5>
                        <h5 className="col-4"> Add/Remove Item </h5>
                        </div>
                        <ul className="menu">
                            {
                                menu
                                    ...
                                    return (
                                        <li key={index}>
                                            <div className="row no-gutters menuItem">
                                                ...
                                                <div className="col-4">
                                                    <button className="btn btn-light mx-2 addButton" onClick={() => this.addItemHandler(item)}>+</button>
                                                    <button className="btn btn-light addButton" onClick={() => this.removeItemHandler(item)}>-</button>
                                                </div>
                                            </div>
                                        </li>
                                    )
